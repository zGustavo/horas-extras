...
vSetor.addEventListener("click", async (e) => {
  try {
    ...
    if (id === "null") {
      const message = "NENHUM FUNCIONÁRIO SELECIONADO";
    ...
    try {
      data = await axios.post(
        "/sistemas/horas-extras/admin/funcionario/create",
        {
          ...
          salEf: Number(inputs[10].value),
          salCom: Number(inputs[11].value),
          tel: inputs[12].value,
          level: Number(inputs[13].value),
        }
      );
